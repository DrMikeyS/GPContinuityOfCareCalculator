...
// Combines appointment CSVs, filters frequent attenders,
...
async function loadData() {
  ...
  // Load and combine data from all uploaded CSV files
  const datasets = await Promise.all(files.map(parseCsv));
  ...
  // Remove clinicians with fewer than 2 sessions
  Object.keys(clinicianSessions).forEach(clinician => {
    if (clinicianSessions[clinician] < 2) {
      delete clinicianSessions[clinician];
    }
  });
...
function assignPatients() {
  ...
  sortedPatients.forEach(([pid]) => {
    ...
    if (!sorted.length) {
      const fallback = Object.keys(clinicianSessions).reduce((min, c) =>
        clinicianCaseloads[c] < clinicianCaseloads[min] ? c : min
      );
      patientAssignments[pid] = fallback;
      clinicianCaseloads[fallback] += 1;
      return;
    }
