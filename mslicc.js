...
document.getElementById('csvFile').addEventListener('change', function(event) {
  ...
  function finalizeProcessing(allData) {
    ...
    const stats = calculateMSLICC(allData, headerMap);
    window.msliccStats = stats;
    displayResults(stats);
    displayTabulator(stats.patientStats, headerMap.patientId);
...
function calculateMSLICC(data, headerMap) {
  ...
  for (const row of data) {
    ...
    if (ageHeader && row[ageHeader] !== undefined && row[ageHeader] !== null && row[ageHeader] !== '') {
      ...
      if (!Number.isNaN(numericAge)) {
        if (!patientAgeMap.has(patientId)) {
  ...
  const measurementMonthStart = new Date(maxDate.getFullYear(), maxDate.getMonth(), 1);
  const measurementMonthEnd = new Date(maxDate.getFullYear(), maxDate.getMonth() + 1, 0, 23, 59, 59, 999);

  const lookbackRecords = records.filter(r => r.appointmentDate < measurementMonthStart);
  const measurementRecords = records.filter(r => r.appointmentDate >= measurementMonthStart && r.appointmentDate <= measurementMonthEnd);
  ...
  return {
    measurementMonthStart,
    measurementMonthEnd,
    measurementLabel: formatMonthLabel(measurementMonthStart),
    numerator,
    denominator,
...
document.getElementById('applyGpFilter').addEventListener('click', function() {
  ...
  const stats = calculateMSLICC(filteredData, window.headerMap);
  window.msliccStats = stats;
  displayResults(stats);
